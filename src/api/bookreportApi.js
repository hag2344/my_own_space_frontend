...
export const bookReportApi = {
  // 리스트 (페이지네이션)
  select(page = 0, size = 20, q = "") {
    return api.get("/bookreport", {
      params: {
        page,
        size,
        q: q?.trim() || undefined,
  ...
  // 단건 조회
  getById(id) {
    return api.get(`/bookreport/${id}`);
  },

  // 생성
  create(data) {
    return api.post("/bookreport", data);
  },

  // 수정
  update(id, data) {
    return api.put(`/bookreport/${id}`, data);
  },

  // 삭제
  delete(id) {
    return api.delete(`/bookreport/${id}`);
  },
  ...
  uploadImage(file) {
    ...
    return api.post("/uploads/image", form, {
      params: { folder: "BOOKREPORT" }, // enum 이름과 맞추기
      headers: {
